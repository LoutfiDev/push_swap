...
/*   Updated: 2023/01/30 20:24:31 by yloutfi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	push_cheapest(t_stack **stack_a, t_stack **stack_b)
{
	...
	while ((*stack_b)->rank != cheap->rank && (*stack_a)->rank != cheap->rank)
	{
		if ((*stack_b)->pos <= ft_lstsize(*stack_b) / 2
			&& (*stack_a)->pos <= ft_lstsize(*stack_a) / 2)
				double_inst("rr", stack_a, stack_b);
		else if ((*stack_b)->pos > ft_lstsize(*stack_b) / 2
			&& (*stack_a)->pos > ft_lstsize(*stack_a) / 2)
				double_inst("rrr", stack_a, stack_b);
	}
	while ((*stack_a)->rank != cheap->rank)
	{
		if ((*stack_a)->pos <= ft_lstsize(*stack_a) / 2)
				rotate(stack_a, "ra\n");
		else if ((*stack_a)->pos > ft_lstsize(*stack_a) / 2)
